...
2. Build the self-contained app:
   ```powershell
   cd ..\..\csharp
   dotnet publish src\SonosStreaming.App -c Release -r win-x64 --self-contained true -p:WindowsAppSDKSelfContained=true -o publish\RoomRelay-v1.0.0
   ```
...
The installer `RoomRelay-Setup-1.0.0.exe` will be created in `csharp\`.
...
```powershell
RoomRelay-Setup-1.0.0.exe /VERYSILENT /NORESTART
```
